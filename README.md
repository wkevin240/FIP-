...
## Facturation vers Accounting

La comptabilisation d’une facture émise est disponible sous `/api/v1/invoicing/invoices/{invoice_id}/post-accounting`. Elle exige une clé `Idempotency-Key`, un profil de comptes configuré par l’organisation et une période ouverte couvrant la date de la facture. Le flux utilise exclusivement `JournalEntryService`, conserve une liaison source unique et écrit l’événement Audit dans la même transaction. Sans profil, compte, journal ou période réel, FIP retourne un état métier explicite et ne génère aucune donnée par défaut. Les détails sont documentés dans [`docs/invoicing_accounting_posting.md`](docs/invoicing_accounting_posting.md).

## Clôture annuelle sécurisée

La pré-clôture disponible sous `/api/v1/accounting/fiscal-years/{id}/closing-preview` vérifie les périodes ouvertes ou verrouillées et les écritures brouillon avant toute clôture. La route de clôture ne modifie l’exercice réel que lorsque tous les contrôles sont passés, puis écrit une trace Audit. FIP ne génère ni nouvel exercice ni solde d’ouverture par défaut. Consultez [`docs/accounting_fiscal_year_closing.md`](docs/accounting_fiscal_year_closing.md) pour les règles détaillées.

## Rapprochement bancaire automatique

Le rapprochement bancaire sous `/api/v1/accounting/bank-reconciliation/automatic` propose puis applique uniquement des correspondances exactes, uniques et de même sens entre transactions bancaires et écritures `POSTED`. Les cas ambigus, incomplets ou sans candidat restent explicitement non rapprochés. Les rapprochements automatiques sont marqués `AUTO_EXACT`, audités dans la transaction et ne créent jamais de transaction, compte ou écriture fictive. Consultez [`docs/accounting_automatic_bank_reconciliation.md`](docs/accounting_automatic_bank_reconciliation.md) pour les critères et limites.
