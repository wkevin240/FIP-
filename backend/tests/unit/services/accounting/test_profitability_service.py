...
async def test_calculate_uses_context_window_and_explicit_mapping() -> None:
    ...
    ledger.reconcile_postings.assert_awaited_once_with(
        "org-1",
...
@pytest.mark.asyncio
async def test_calculate_refuses_unreconciled_ledger() -> None:
    ledger = AsyncMock()
    ledger.reconcile_postings.return_value = {
        "is_reconciled": False,
        "missing_postings": ["line-1"],
        "orphan_postings": [],
        "mismatched_postings": [],
    }
    service = LedgerProfitabilityService(ledger)
    ...
    with pytest.raises(LedgerProfitabilityIntegrityError, match="unreconciled ledger slice"):
