        ...
        try:
            async with connection.transaction():
                await connection.execute(
                    ...
                    VALUES ('period-guard-org', 'integration-only', TRUE, NOW(), NOW())
                    """
                )
                await connection.execute(
                    """
                ...
                await connection.execute(
                    ...
                    VALUES ('period-guard-closed', 'January 2026', '2026-01-01', '2026-01-31', 'CLOSED', 'period-guard-year', 'period-guard-org', NOW(), NOW())
                    """
                )
                await connection.execute(
                    """
                    INSERT INTO accounts (id, organization_id, code, name, is_active, account_type, level, path, created_at, updated_at)
                    VALUES ('period-guard-account', 'period-guard-org', 'TEST-PG', 'integration-only', TRUE, 'EXPENSE', 1, '/', NOW(), NOW())
                    """
                )
                ...
                await connection.execute(
                    """
                    INSERT INTO fiscal_periods (id, name, start_date, end_date, status, fiscal_year_id, organization_id, created_at, updated_at)
                    VALUES ('period-guard-open', 'February 2026', '2026-02-01', '2026-02-28', 'OPEN', 'period-guard-year', 'period-guard-org', NOW(), NOW())
                    """
                )
                await connection.execute(
                    """
                ...
                await connection.execute(
                    "UPDATE fiscal_periods SET status = 'CLOSED' WHERE id = 'period-guard-open'"
                )
