...
class ScenarioService:
    ...
    async def create(
        self, organization_id: str, actor_user_id: str, data: ScenarioCreate
    ) -> Scenario:
        scenario = Scenario(
        ...
        try:
            ...
            await self.audit.record(
                organization_id,
                actor_user_id,
                "FPA_SCENARIO_CREATED",
                "Scenario",
                scenario.id,
                new_value={
                    "code": scenario.code,
                    "fiscal_year_id": scenario.fiscal_year_id,
                },
            )
            ...
            raise HTTPException(
                ...
                detail="Scenario code already exists for this organization and fiscal year",
            )
        return scenario
    ...
    async def add_assumption(
        ...
        if scenario.status != "DRAFT":
            raise HTTPException(
                status_code=409, detail="Only DRAFT scenarios can be changed"
            )
        period = await self.session.scalar(
            select(FiscalPeriod).where(
                FiscalPeriod.organization_id == organization_id,
                FiscalPeriod.id == data.fiscal_period_id,
                FiscalPeriod.fiscal_year_id == scenario.fiscal_year_id,
            )
        )
        if period is None:
            raise HTTPException(
                status_code=422,
                detail="Fiscal period does not belong to the scenario fiscal year",
            )
        ...
        assumption = ScenarioAssumption(
            ...
            dimension_value_id=data.dimension_value_id,
            amount=data.amount,
        ...
        try:
            ...
            raise HTTPException(
                status_code=409,
                detail="An assumption already exists for this scenario scope",
            )
        return assumption

    async def approve(
        self, organization_id: str, actor_user_id: str, scenario_id: str
    ) -> Scenario:
        scenario = await self._get(organization_id, scenario_id, for_update=True)
        ...
        if scenario.status != "DRAFT":
            raise HTTPException(
                status_code=409, detail="Locked scenario cannot be approved"
            )
        count = await self.session.scalar(
            select(func.count(ScenarioAssumption.id)).where(
        ...
        if not count:
            raise HTTPException(
                status_code=422,
                detail="A scenario requires at least one explicit assumption",
            )
        ...
        await self.audit.record(
            organization_id,
            actor_user_id,
            "FPA_SCENARIO_APPROVED",
            "Scenario",
            scenario.id,
            new_value={"status": scenario.status, "assumption_count": count},
        )
        await self.session.commit()
        return scenario
    ...
    async def status(self, organization_id: str, scenario_id: str) -> dict:
        ...
        return {
            ...
            "assumption_count": int(count or 0),
            "ready": scenario.status in {"APPROVED", "LOCKED"} and bool(count),
        }

    async def _get(
        self, organization_id: str, scenario_id: str, for_update: bool = False
    ) -> Scenario:
        query = select(Scenario).where(
            Scenario.organization_id == organization_id, Scenario.id == scenario_id
        )
        if for_update:
