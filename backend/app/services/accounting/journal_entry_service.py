...
class JournalEntryService:
    ...
    async def create(self, organization_id: str, data: JournalEntryCreate) -> JournalEntry:
        ...
        entry = JournalEntry(organization_id=organization_id, fiscal_period_id=data.fiscal_period_id, entry_date=data.entry_date, reference=data.reference.strip() if data.reference else None, description=data.description.strip(), idempotency_key=data.idempotency_key, idempotency_hash=request_hash, status=JournalEntryStatus.DRAFT)
        entry.lines = [JournalEntryLine(line_number=index, account_id=line.account_id, description=line.description.strip() if line.description else None, debit=line.debit, credit=line.credit) for index, line in enumerate(data.lines, start=1)]
    ...
    async def post(self, organization_id: str, entry_id: str, actor_id: str) -> JournalEntry:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=str(exc)) from exc
        existing = await self.session.scalar(select(LedgerPosting.id).where(LedgerPosting.organization_id == organization_id, LedgerPosting.journal_entry_id == entry.id).limit(1))
        if existing:
            raise HTTPException(status_code=409, detail="Journal entry already has ledger postings")
        for line in entry.lines:
            self.session.add(LedgerPosting(organization_id=entry.organization_id, fiscal_period_id=entry.fiscal_period_id, journal_entry_id=entry.id, journal_entry_line_id=line.id, account_id=line.account_id, posting_date=entry.entry_date, line_number=line.line_number, description=line.description, debit=line.debit, credit=line.credit))
    ...
    async def reverse(self, organization_id: str, entry_id: str, actor_id: str, data: JournalEntryReverse) -> JournalEntry:
        ...
        reversal = JournalEntry(organization_id=organization_id, fiscal_period_id=original.fiscal_period_id, entry_date=reversal_date, reference=data.reference or original.reference, description=data.description or f"Reversal of journal entry {original.id}", status=JournalEntryStatus.DRAFT, idempotency_key=data.idempotency_key, idempotency_hash=request_hash, reversal_of_id=original.id)
        reversal.lines = [JournalEntryLine(line_number=index, account_id=line.account_id, description=line.description, debit=line.credit, credit=line.debit) for index, line in enumerate(original.lines, start=1)]
        ...
        original = await self.session.scalar(select(JournalEntry).where(JournalEntry.organization_id == organization_id, JournalEntry.id == original.id))
