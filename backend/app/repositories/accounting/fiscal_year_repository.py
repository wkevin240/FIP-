...
class FiscalYearRepository:
    ...
    async def get_by_id(self, organization_id: str, fiscal_year_id: str) -> FiscalYear | None:
        return await self.session.scalar(select(FiscalYear).where(FiscalYear.organization_id == organization_id, FiscalYear.id == fiscal_year_id))

    async def list(self, organization_id: str) -> list[FiscalYear]:
        result = await self.session.scalars(select(FiscalYear).where(FiscalYear.organization_id == organization_id).order_by(FiscalYear.start_date))
        return list(result)
    ...
    async def update(self, fiscal_year: FiscalYear, data: FiscalYearUpdate) -> FiscalYear:
